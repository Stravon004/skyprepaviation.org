...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (data) {
        ...
        if (data.length > 0) {
          const avg = data.reduce((acc, s) => acc + (s.correct_answers / s.total_questions) * 100, 0) / data.length
  ...
  const isStudent = profile?.subscription_tier === 'student' || isPro

  const quickActions = [
    { to: '/exam', icon: BookOpen, label: 'Start Practice Exam', desc: 'Full-length timed test', color: 'sky', locked: false },
    { to: '/flashcards', icon: Brain, label: 'Review Flashcards', desc: 'Spaced repetition study', color: 'emerald', locked: false },
    { to: '/oral-sim', icon: MessageSquare, label: 'AI Oral Simulator', desc: 'DPE-style questioning', color: 'amber', locked: !isStudent },
  ]
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Exams Taken', val: loading ? '—' : totalSessions, icon: CheckCircle, color: 'text-sky-400' },
          ...
          {quickActions.map(({ to, icon: Icon, label, desc, locked }) => (
            <div key={to} className="relative">
              {locked ? (
                ...
                <Link to={to} className="card group hover:border-slate-700 hover:-translate-y-0.5 transition-all duration-200 block">
                  <div className="flex items-center justify-between mb-3">
                    <Icon size={20} className="text-sky-400 group-hover:text-sky-300 transition-colors" />
            ...
            {recentSessions.map(s => {
              ...
              return (
                <Link key={s.id} to={`/results/${s.id}`} className="flex items-center justify-between px-6 py-4 hover:bg-slate-800/50 transition-colors group">
                  ...
                  <div className="flex items-center gap-4">
                    <span className={`text-lg font-bold ${passed ? 'text-emerald-400' : 'text-red-400'}`}>{pct}%</span>
