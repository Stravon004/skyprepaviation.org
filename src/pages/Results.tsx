...
export default function Results() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: qs } = await supabase.from('questions').select('id, question, options, correct_answer, explanation, category').in('id', ids)
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto animate-fade-in">
      ...
      <div className={`card mb-8 text-center ${passed ? 'border-emerald-500/30 bg-emerald-900/10' : 'border-red-500/30 bg-red-900/10'}`}>
        ...
        <div className={`text-xl font-semibold mb-1 ${passed ? 'text-emerald-300' : 'text-red-300'}`}>{passed ? 'PASSED' : 'NOT PASSED'}</div>
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          {[
            { val: session.certificate, label: 'Certificate' },
            { val: session.correct_answers, label: 'Correct' },
            { val: session.total_questions - session.correct_answers, label: 'Incorrect' },
          ].map(({ val, label }) => (
      ...
      <div className="flex gap-3 mb-8">
        <Link to="/exam" className="btn-primary flex items-center gap-2 text-sm"><RotateCcw size={15} />Take Another Exam</Link>
            ...
            return (
              <div key={result.questionId} className={`card border ${result.correct ? 'border-emerald-800/40' : 'border-red-800/40'}`}>
                <div className="flex items-start gap-3 mb-4">
                  {result.correct ? <CheckCircle size={18} className="text-emerald-400 flex-shrink-0 mt-0.5" /> : <XCircle size={18} className="text-red-400 flex-shrink-0 mt-0.5" />}
                  <div>
                ...
                <div className="space-y-1.5 mb-4 ml-7">
                  {q.options.map((opt, oi) => {
                    ...
                    return (
                      <div key={oi} className={`text-sm px-3 py-2 rounded-lg flex items-center gap-2 ${isCorrect ? 'bg-emerald-500/10 text-emerald-300' : isSelected && !isCorrect ? 'bg-red-500/10 text-red-300' : 'text-slate-500'}`}>
