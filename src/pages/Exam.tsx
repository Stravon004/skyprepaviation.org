    ...
    return (
      <div className="p-8 max-w-2xl mx-auto animate-fade-in">
        ...
        <div className="card space-y-6">
              ...
              {(['PPL', 'IFR', 'CPL', 'ATP'] as Cert[]).map(cert => {
                ...
                return (
                  ...
                  >
                    ...
                    {locked && <span className="absolute top-2 right-2 badge bg-amber-500/10 text-amber-500 border border-amber-500/20">Pro</span>}
                  </button>
                )
              })}
            </div>
          ...
          {loadError && (
            <div className="bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl px-4 py-3 text-sm">{loadError}</div>
          )}
          ...
          <div className="bg-slate-800/50 rounded-xl p-4 text-sm text-slate-400 space-y-1">
            ...
            <p>• Review answers and explanations after submission</p>
          </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="border-b border-slate-800 bg-slate-900 px-6 py-4 flex items-center justify-between gap-4">
        ...
        <div className="flex-1 max-w-xs">
          <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full bg-sky-600 rounded-full transition-all duration-300" style={{ width: `${progress}%` }} />
          </div>
        </div>
        ...
        <button
          ...
          className={`flex items-center gap-1.5 text-sm font-medium px-3 py-1.5 rounded-lg transition-all ${
            flagged.has(currentIdx) ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'
          }`}
        >
          <Flag size={14} />
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-3xl mx-auto">
          <div className="mb-2">
            <span className="text-xs text-slate-500 font-medium uppercase tracking-wide">{q.category}</span>
          </div>
                ...
                >
                  <div className="flex items-start gap-3">
                    <span className={`w-6 h-6 rounded-full border flex-shrink-0 flex items-center justify-center text-xs font-bold mt-0.5 ${
                      selected ? 'border-sky-500 bg-sky-600 text-white' : 'border-slate-600 text-slate-500'
                    }`}>
      ...
      <div className="border-t border-slate-800 bg-slate-900 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex gap-1 max-w-md overflow-x-auto">
          {questions.slice(Math.max(0, currentIdx - 4), Math.min(questions.length, currentIdx + 5)).map((_, relIdx) => {
            const absIdx = Math.max(0, currentIdx - 4) + relIdx
            const isAnswered = absIdx in answers
            const isFlagged = flagged.has(absIdx)
            const isCurrent = absIdx === currentIdx
            return (
              <button
                key={absIdx}
                onClick={() => setCurrentIdx(absIdx)}
                className={`w-7 h-7 rounded text-xs font-semibold flex-shrink-0 transition-all ${
                  isCurrent ? 'bg-sky-600 text-white' :
                  isFlagged ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' :
        ...
        <button
          onClick={() => currentIdx === questions.length - 1 ? submitExam() : setCurrentIdx(i => i + 1)}
